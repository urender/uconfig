...
let GeneratorProto = {
	...
	emit_object_tests: function(indent, valueExpr, valueSpec)
	{
		...
		if (exists(valueSpec, 'patternProperties')) {
			for (let pattern, val in valueSpec.patternProperties) {
				let fn = to_method_name('instantiate', replace(val['$ref'], '#/$defs/', ''));
				this.print(indent, '	for (let name, object in value)');
				this.print(indent, '		if (match(name, regexp(name)))');
				this.print(indent, '			obj[name] = %s(location + "/" + name, object, errors);\n', fn)
			}
		}
	...
	print: function(indent, fmt, ...args)
	{
		if (length(fmt)) {
			...
			this.output.write(sprintf(fmt, ...args));
		}
	...
	{
		let moduleName = to_method_name('', propertyName);
		let functionName = 'instantiate' + moduleName;
		let indent = '';

		modules[propertyName] = valueSpec;
	...
	{
		if (!this.generate_modules && valueSpec['$module'])
			return this.emit_include_validation_function(propertyName, valueSpec);
	...
	generate: function(file)
	{
		...
		this.emit_spec_validation_function(indent, 'new', "UConfigState", this.schema);

		this.print(indent, 'export function validate(value, errors) {');
		this.print(indent, '	let err = [];');
		this.print(indent, '	let res = newUConfigState("", value, err);');
	...
	module: function(module)
	{
		...
		this.print(indent, '	validate: function(location, value, errors) {');
		this.print(indent, '		return ' + functionName + '(location, value, errors);');
		this.print(indent, '	}');
		this.print(indent, '};');
